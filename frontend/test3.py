...
# Convert time for UNIX timestamp to UTC time
...
def display_filtered_incident_details(id):
...
def display_validated_incident_details(id):
...
def display_map_and_filtered_incidents_table():
    with st.container(key="pydeck_map"):
        ...
        # Get colour-code of incident
        def get_colour(incident):
            if incident["type"] == "Filtered":
                if 0.7 < incident["priority"] <= 1: # Return opaque red
                    return [255, 0, 0, 255]
                ...
                opacity = 50
                if 0.7 < incident["priority"] <= 1: # Return transluscent red
        ...
        def parse_filtered(incident):
            return {
                "longitude": incident["crowdsource_event"]["x"],
                "latitude": incident["crowdsource_event"]["y"],
                "alert_subtype": (incident["crowdsource_event"]["alert_subtype"]
                                    if incident["crowdsource_event"]["alert_subtype"]
                                    else "Unknown"),
        ...
        incidents = [parse_filtered(incident) for incident in st.session_state["filtered"]]
        ...
        st.pydeck_chart(pdk.Deck(
            ...
            tooltip=tooltip,
        ), use_container_width=False, width=1720) # Centralise map in the container because for some reason it is not done by default
    ...
    with st.container(key="filtered_cont"):
        ...
        if not st.session_state["filtered"]:
            ...
            # Store incidents that are already in the table
            if "parsed_filtered" not in st.session_state:
                st.session_state["parsed_filtered"] = []
            ...
            for incident in st.session_state["filtered"]:
                if incident["id"] not in all_ids: # If incident is not in table yet
                    st.session_state["parsed_filtered"].append(parse_incoming(incident))
...
def display_incoming_incidents_table():
    ...
    if not st.session_state["incoming"]:
        ...
        df["S/N"] = ["🚨 No Records Found"] # Initialise serial number value as default value
        df = df[["S/N"] + table_col] # Reorder columns
        st.data_editor(df, use_container_width=True, key="parsed_incoming_incidents_editor", disabled=["S/N"] + table_col, hide_index=True)
        ...
        df.index.name = "S/N" # Rename index column
        st.data_editor(df, use_container_width=True, key="parsed_incoming_incidents_editor", disabled=df.columns.tolist()) # Display table. Used over st.dataframe because this handles adjusting to the column width. 
...
def main():
    ...
    if st.session_state["selected_filtered"]:
        display_filtered_incident_details(st.session_state["selected_filtered"])               


    # If user wants to view details of a validated incident i.e. a checkbox is checked
    elif st.session_state["selected_validated"]:
        display_validated_incident_details(st.session_state["selected_validated"])
    ...
    # If user does nothing i.e. view homescreen
        ...
        with working:
            display_map_and_filtered_incidents_table()
        ...
        with archived:
            display_incoming_incidents_table()
